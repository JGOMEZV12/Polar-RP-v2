...
namespace Polar.HabboHotel.Items.Data.Moodlight
{
    public class MoodlightData
    {
        ...
        public void UpdatePreset(int Preset, string Color, int Intensity, bool BgOnly, bool Hax = false)
        {
            ...
            using (IQueryAdapter dbClient = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("UPDATE room_items_moodlight SET preset_" + Pr + " = '@color," + Intensity + "," + PolarEnvironment.BoolToEnum(BgOnly) + "' WHERE item_id = '" + ItemId + "' LIMIT 1");
        ...
        public static MoodlightPreset GeneratePreset(string Data)
        {
            ...
            return new MoodlightPreset(Bits[0], int.Parse(Bits[1]), PolarEnvironment.EnumToBool(Bits[2]));
        }
