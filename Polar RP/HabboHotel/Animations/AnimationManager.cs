...
namespace Polar.HabboHotel.Animations
{
    public class AnimationManager
    {
        ...
        private void Cycle()
        {
            ...
            if (this._started)
            {
                ...
                {
                    Room Rooom;
                    if (PolarEnvironment.GetGame().GetRoomManager().LoadRoom(this._roomIdGame, out Rooom) && Rooom != null)
                    {
            ...
            {
                this._notif = true;
                //PolarEnvironment.GetGame().GetClientWebManager().SendMessage(new NotifTopComposer("Jack & Daisy: La prochaine animation démarre dans 2 minutes !"), Core.Language.FRANCAIS);
            }
            ...
            if (this._timer >= this.GetMinutes(START_TIME))
            {
                ...
                Room room;
                if (!PolarEnvironment.GetGame().GetRoomManager().LoadRoom(RoomId, out room) || room == null)
                    return;
                ...
                string event_alert = (PolarEnvironment.GetConfig().data["event_alert"]);
