...
namespace Polar.HabboHotel.Support
{
    /// <summary>
    ...
    public class ModerationTool
    {
        ...
        public void SendNewTicket(GameClient Session, int Category, int ReportedUser, String Message, List<string> Messages)
        {
            ...
            if (Session.GetHabbo().CurrentRoomId <= 0)
            {
                ...
                {
                    dbClient.SetQuery("INSERT INTO moderation_tickets (score,type,status,sender_id,reported_id,moderator_id,message,room_id,room_name,timestamp) VALUES (1,'" + Category + "','open','" + Session.GetHabbo().Id + "','" + ReportedUser + "','0', @message,'0','','" + PolarEnvironment.GetUnixTimestamp() + "')");
                ...
                Ticket = new SupportTicket(TicketId, 1, 7, Category, Session.GetHabbo().Id, ReportedUser, Message, 0, "", PolarEnvironment.GetUnixTimestamp(), Messages);
            ...
            RoomData Data = PolarEnvironment.GetGame().GetRoomManager().GenerateRoomData(Session.GetHabbo().CurrentRoomId);

            using (IQueryAdapter dbClient = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("INSERT INTO moderation_tickets (score,type,status,sender_id,reported_id,moderator_id,message,room_id,room_name,timestamp) VALUES (1,'" + Category + "','open','" + Session.GetHabbo().Id + "','" + ReportedUser + "','0', @message,'" + Data.Id + "', @name,'" + PolarEnvironment.GetUnixTimestamp() + "')");
                dbClient.AddParameter("message", Message);
            ...
            Ticket = new SupportTicket(TicketId, 1, 7, Category, Session.GetHabbo().Id, ReportedUser, Message, Data.Id, Data.Name, PolarEnvironment.GetUnixTimestamp(), Messages);
            Tickets.Add(Ticket);
            SendTicketToModerators(Ticket);
        }
        ...
        public void CloseTicket(GameClient Session, int TicketId, int Result)
        {
            ...
            GameClient Client = PolarEnvironment.GetGame().GetClientManager().GetClientByUserID(Ticket.SenderId);
        ...
        {
            PolarEnvironment.GetGame().GetClientManager().SendMessage(new ModeratorSupportTicketComposer(Ticket), "mod_tool");
        }
