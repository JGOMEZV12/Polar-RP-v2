...
namespace Polar.HabboHotel.Support
{
    ...
    public class SupportTicket
    {
        ...
        public SupportTicket(int Id, int Score, int Type, int Category, int SenderId, int ReportedId, String Message, int RoomId, String RoomName, Double Timestamp, List<string> ReportedChats)
        {
            ...
            SenderName = PolarEnvironment.GetGame().GetClientManager().GetNameById(SenderId);
            ReportedName = PolarEnvironment.GetGame().GetClientManager().GetNameById(ReportedId);
            ModName = PolarEnvironment.GetGame().GetClientManager().GetNameById(ModeratorId);
        }
        ...
        public void Pick(int pModeratorId, Boolean UpdateInDb)
        {
            ...
            ModName = PolarEnvironment.GetHabboById(pModeratorId).Username;
            if (UpdateInDb)
            {
                using (IQueryAdapter dbClient = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.RunQuery("UPDATE moderation_tickets SET status = 'picked', moderator_id = " + pModeratorId + ", timestamp = '" + PolarEnvironment.GetUnixTimestamp() + "' WHERE id = " + Id + "");
                }
            }
        }
