...
{
    class TeamoCommand : IChatCommand
    {
        ...
        {
            get { return "Dile a otra persona que la amas"; }
        }
        ...
        public async Task Execute(GameClients.GameClient Session, Rooms.Room Room, string[] Params)
        {
            ...
            if (Session.GetRoleplay().TryGetCooldown("teamo"))
                return;

            if (TargetUser.IsAsleep)
            {
                Session.SendWhisper("No se puede decirle te amo a alguien que no está jugando el juego ahora mismo", 1);
                return;
            }
            #endregion
            ...
            if (Distance <= 1)
            {
                Session.Shout("*Hey " + TargetClient.GetHabbo().Username + " yo TE AMO |*", 16);
                Session.GetRoleplay().CooldownManager.CreateCooldown("teamo", 1000, 20);
            ...
            {
                Session.SendWhisper("¡Debes acercarte a este ciudadano para decirle que lo/la amas!", 1);
                return;
            }
            #endregion
        }
