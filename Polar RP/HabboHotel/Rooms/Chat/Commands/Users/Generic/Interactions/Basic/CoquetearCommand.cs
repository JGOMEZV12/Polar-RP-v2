...
{
    class CoquetearCommand : IChatCommand
    {
        ...
        {
            get { return "Conquetea con otro usuario"; }
        }
        ...
        public async Task Execute(GameClients.GameClient Session, Rooms.Room Room, string[] Params)
        {
            ...
            if (Session.GetRoleplay().TryGetCooldown("coquetear"))
                return;

            if (TargetUser.IsAsleep)
            {
                Session.SendWhisper("No se puede coquetear a alguien que no está jugando el juego ahora mismo", 1);
                return;
            }
            #endregion
            ...
            if (Distance <= 1)
            {
                Session.Shout("*Se acerca hacia " + TargetClient.GetHabbo().Username + " Y le dice un piropo al oído*", 16);
                TargetClient.Shout("* Se sonroja porque le dijeron algo lindo al oído *");
                TargetClient.GetRoomUser().ApplyEffect(910);
                Session.GetRoleplay().CooldownManager.CreateCooldown("coquetear", 1000, 5);
                TargetClient.GetRoleplay().KissTimer = 5;
                return;
            }
            else
            {
                Session.SendWhisper("¡Debes acercarte a este ciudadano para coquetear!", 1);
                return;
            }
            #endregion
        }
