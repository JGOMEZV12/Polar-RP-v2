...
{
    class CtransferirCommand : IChatCommand
    {
        ...
        public async Task Execute(GameClients.GameClient Session, Rooms.Room Room, string[] Params)
        {
           ...
           /* if (TargetClient.MachineId == Session.MachineId)
            ...
            if (TargetClient.GetRoleplay().BankAccount < 1)
            {
                Session.SendWhisper("¡Esta persona no tiene cuenta corriente!", 1);
                return;
            }
            ...
            if (int.TryParse((Params[2]), out Amount))
            {
                ...
                if (Session.GetRoleplay().BankChequings < 0)
                {
                ...
                {
                    Session.SendWhisper("Usted no posee ninguna cuenta corriente, vaya al banco [Sala: 53] y abra una", 1);
                    return;
                }

                if (Session.GetRoleplay().BankChequings < 30000)
                {
                ...
                TargetClient.GetRoleplay().BankChequings += Amount;
                Session.Shout("*Transfiere de su cuenta CORRIENTE: $" + String.Format("{0:N0}", Amount) + " a " + TargetClient.GetHabbo().Username + " *", 4);
