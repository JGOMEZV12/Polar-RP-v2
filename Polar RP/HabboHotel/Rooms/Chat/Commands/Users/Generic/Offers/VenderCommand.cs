...
{
    class VenderCommand : IChatCommand
    {
        ...
        public async Task Execute(GameClients.GameClient Session, Rooms.Room Room, string[] Params)
        {
            ...
            {
                Session.SendWhisper("Su nivel es demasiado bajo para vender armas a particulares, se requiere nivel 10", 1);
                return;
            }

            /*if (Target.MachineId == Session.MachineId)
            {
                Session.SendWhisper("¡No puedes vender arma a otra de tus cuentas!", 1);
                return;
            }*/
            ...
            int Cost = Convert.ToInt32(Params[3].ToString().Replace("-", ""));
            bool HasOffer = false;
            if (Target.GetHabbo().Credits >= Cost)
            {
                ...
                if (!HasOffer)
                {
                    ...
                    Target.SendWhisper("Acaba de ofrecerte a " + weapon.PublicName + " por $" + String.Format("{0:N0}", Cost) + "! Diga ':acc arma' para comprarlo!", 1);
                    return;
                }
                else
                {
            ...
            {
                Session.SendWhisper("Este ciudadano no puede pagar una " + weapon.PublicName + "!", 1);
                return;
            }
        }
    }
