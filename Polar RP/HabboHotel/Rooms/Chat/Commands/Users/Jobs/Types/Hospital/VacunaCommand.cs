...
{
    class VacunaCommand : IChatCommand
    {
        ...
        {
            get { return "Quitale el sida a la persona que lo necesite"; }
        }
        ...
        public async Task Execute(GameClient Session, Rooms.Room Room, string[] Params)
        {
            ...
            if (!GroupManager.HasJobCommand(Session, "vacuna"))
            {
            ...
            {
                Session.SendWhisper("¡No puedes quitar sida a alguien que está muerto!", 1);
                return;
            }

            if (TargetUser.IsAsleep)
            {
                Session.SendWhisper("¡No puedes quitar sida a alguien que no está jugando ahora mismo!", 1);
                return;
            }

            if (TargetClient.GetHabbo().Credits < 150)
            {
                Session.SendWhisper("¡No tiene 150$ para pincharlo!", 1);
                return;
            }

            if (Session.GetRoomUser().CarryItemID < 1014)
            {
                Session.SendWhisper("¡Ve al armario de Armario de medicamentos y saca una inyectadora para poder vacunar!", 1);
                return;
            }

            if (TargetClient.GetRoleplay().Sida < 100)
            {
                Session.SendWhisper("¡Aún no esta tan grave como para aplicarle este tratamiento!", 1);
                return;
            }
            ...
            if (Distance <= 2)
            {
                ...
                Session.Shout("* Inyecta a " + TargetClient.GetHabbo().Username + " Poniendole 5Cc de un tratamiento para tratar su enfermedad *", 15);
                TargetClient.Shout("*" + TargetClient.GetHabbo().Username + " Se siente mejor gracias al tratamiento [-100 SIDA] [-1000$]*", 5);
                TargetClient.GetHabbo().Credits -= 1000;
                TargetClient.GetHabbo().UpdateCreditsBalance();
                TargetClient.GetRoleplay().Sida -= 100;
                TargetClient.GetRoleplay().TimerManager.CreateTimer("pinchar", 4000, false);
                #region Bank Company Balance
                RoleplayManager.GiveMoneyToCompany(2, Session, "hospital", true, 1000);
