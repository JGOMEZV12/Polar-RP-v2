...
namespace Polar.HabboHotel.Rooms.Chat.Commands.Managers
{
    class AccountCheckCommand :IChatCommand
    {
        ...
        {
            get { return "Comprueba los usuarios de otras cuentas basadas en MAC e IP."; }
        }
        ...
        public async Task Execute(GameClients.GameClient Session, Rooms.Room Room, string[] Params)
        {
            if (Params.Length != 3)
            {
                Session.SendWhisper("Por favor, ingrese el nombre de usuario del usuario que desea comprobar y si es ip check o no.", 1);
                return;
            }
            ...
            using (IQueryAdapter dbClient = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (TargetClient == null)
                {
                    ...
                    if (Row == null)
                    {
                        Session.SendWhisper("No existe tal persona", 1);
                        return;
                    }
                ...
                {
                    Session.SendWhisper("Por alguna extraña razón, la base de datos no contiene datos sobre este usuario.", 1);
                    return;
                }

                StringBuilder Message = new StringBuilder();
                Message.Append("----- " + Username + "'s otras cuentas -----\n");
                Message.Append("[ID] --- USERNAME\n\n");
