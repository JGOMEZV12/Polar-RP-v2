...
{
    class PatearCommand : IChatCommand
    {
        ...
        {
            get { return "Patea a otro usuario."; }
        }
        ...
        public async Task Execute(GameClients.GameClient Session, Rooms.Room Room, string[] Params)
        {
            ...
            {
                Session.SendWhisper("¡No puedes patear a alguien mientras estás de servicio!", 1);
                return;
            }
            ...
            {
                Session.SendWhisper("¡No puedes patearte!", 1);
                return;
            }

            if (TargetUser.IsAsleep)
            {
                Session.SendWhisper("¡No puedes patear a alguien que no está jugando el juego ahora mismo!", 1);
                return;
            }
            ...
            {
                Session.SendWhisper("¡No puedes patear a un embajador que está de servicio!", 1);
                return;
            }

            if (Session.GetRoleplay().TryGetCooldown("patear"))
                return;
            ...
            if (!((Math.Abs(TargetUser.X - ThisUser.X) >= 2) || (Math.Abs(TargetUser.Y - ThisUser.Y) >= 2)))
            {
                ...
                Session.Shout("*Patea a " + TargetClient.GetHabbo().Username + " ¡Largate!*");
                Session.GetRoleplay().CooldownManager.CreateCooldown("patear", 1000, 20);
