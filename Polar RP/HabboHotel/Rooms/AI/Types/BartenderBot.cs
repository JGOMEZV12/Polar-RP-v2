...
namespace Polar.HabboHotel.Rooms.AI.Types
{
    class BartenderBot : BotAI
    {
        ...
        public override void OnUserSay(RoomUser User, string Message)
        {
            ...
            BotResponse Response = PolarEnvironment.GetGame().GetBotManager().GetResponse(GetBotData().AiType, Message);
            if (Response == null)
                return;
        ...
        public override void OnTimerTick()
        {
            ...
            if (SpeechTimer <= 0)
            {
                if (GetBotData().RandomSpeech.Count > 0)
                {
                    ...
                    string word;
                    string String = PolarEnvironment.GetGame().GetChatManager().GetFilter().IsUnnaceptableWord(Speech.Message, out word) ? "Spam" : Speech.Message;
                    if (String.Contains("<img src") || String.Contains("<font ") || String.Contains("</font>") || String.Contains("</a>") || String.Contains("<i>"))
                        String = "Realmente no debería usar HTML dentro de los discursos de bot.";
