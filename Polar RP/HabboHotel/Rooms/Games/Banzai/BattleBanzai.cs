...
using Polar.Utilities.Enclosure;
using System.Linq;
...
namespace Polar.HabboHotel.Rooms.Games.Banzai
{
    public class BattleBanzai
    {
        ...
        public void BanzaiStart()
        {
            ...
            timestarted = PolarEnvironment.GetUnixTimestamp();
        ...
        public void BanzaiEnd(bool userTriggered = false)
        {
            ...
            if (winners != TEAM.NONE)
            {
                ...
                foreach (RoomUser User in Winners.ToList())
                {
                    ...
                    {
                        //if (PolarEnvironment.GetUnixTimestamp() - timestarted > 5)
                        {
                            PolarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallTilesLocked", User.LockedTilesCount);
                            PolarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallPlayer", 1);
                        }
                    }
                    if (winners == TEAM.BLUE)
                    {
                        ...
                        {
                            //if (PolarEnvironment.GetUnixTimestamp() - timestarted > 5)
                                PolarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallWinner", 1);
        ...
        public void MovePuck(Item item, GameClient mover, int newX, int newY, TEAM team)
        {
            ...
            RoomUser user = mover.GetRoomUser();
            if (banzaiStarted)
            {
