...
namespace Polar.HabboHotel.Achievements
{
    ...
    public class TalentManager
    {
        ...
        public void CompleteUserTalent(GameClient session, Talent talent)
        {
            ...
            {
                ItemData item = null;
                if (!PolarEnvironment.GetGame().GetItemManager().GetItem(talent.PrizeBaseItem, out item))
                    return;
            ...
            if (talent.Type == "citizenship")
            {
                ...
                {
                    case 3:
                        PolarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(session, "ACH_Citizenship", 1);
                        break;
                    case 4:
                        ...
                        using (IQueryAdapter queryReactor = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
                        {
                            queryReactor.RunQuery(string.Concat(new object[]{"UPDATE users SET talent_status = 'helper' WHERE id = ",session.GetHabbo().Id,";"}));
                        }
                        break;
                }
            }
