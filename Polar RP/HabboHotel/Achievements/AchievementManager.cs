...
namespace Polar.HabboHotel.Achievements
{
    public class AchievementManager
    {
        private static readonly ILog log = LogManager.GetLogger("Polar.HabboHotel.Achievements.AchievementManager");
        ...
        public AchievementManager()
        {
            ...
            Out.WriteLine("Administrador de Logros -> CARGADO!", "Polar.HabboHotel", ConsoleColor.DarkGray);
        }
        ...
        public void TryProgressLoginAchievements(GameClient Session)
        {
            ...
            Double daysBtwLastLogin = PolarEnvironment.GetUnixTimestamp() - Session.GetHabbo().LastOnline;
        ...
        public void TryProgressRegistrationAchievements(GameClient Session)
        {
            ...
            double sinceMember = PolarEnvironment.GetUnixTimestamp() - (int)Session.GetHabbo().AccountCreated;
        ...
        public void TryProgressHabboClubAchievements(GameClient Session)
        {
            ...
            Double SinceActivation = PolarEnvironment.GetUnixTimestamp() - Subscription.ExpireTime;
        ...
        public bool ProgressAchievement(GameClient Session, string AchievementGroup, int ProgressAmount, bool FromZero = false)
        {
            ...
            if (NewProgress >= TargetLevelData.Requirement)
            {
                ...
                Session.GetHabbo().Credits += TargetLevelData.RewardPixels;
                Session.GetHabbo().UpdateCreditsBalance();
                Session.GetHabbo().GetStats().AchievementPoints += TargetLevelData.RewardPoints;
                Session.SendMessage(new HabboActivityPointNotificationComposer(Session.GetHabbo().Credits, TargetLevelData.RewardPixels));
                ...
                // Set Talent
                Talent talent = null;
                if (PolarEnvironment.GetGame().GetTalentManager().TryGetTalent(AchievementGroup, out talent))
                    PolarEnvironment.GetGame().GetTalentManager().CompleteUserTalent(Session, talent);
                return true;
            }
            else
            {
