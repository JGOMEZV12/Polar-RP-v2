...
namespace Polar.Communication.Packets.Incoming.LandingView
{
    public class GetHallOfFame
    {
        ...
        public List<UserCompetition> usersWithRank { get; private set; }
        ...
        public void Load()
        {
            ...
            using (IQueryAdapter dbQuery = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                dbQuery.SetQuery("SELECT * FROM `users`, `rp_stats` WHERE `rp_stats`.`level` >= '1' AND `users`.`id` = `rp_stats`.`id` ORDER BY `rp_stats`.`level` DESC LIMIT 16");
        ...
        public void Serialize(ServerPacket message)
        {
            ...
            foreach (UserCompetition user in usersWithRank)
            {
                ...
                message.WriteInteger((int)user.Score);//?
            }
        }
    }
    ...
    public class UserCompetition
    {
        ...
        public UserCompetition(DataRow row)
        {
            ...
            this.Score = Convert.ToInt32(row["level"].ToString());
        }
    }
