...
namespace Polar.Communication.Packets.Incoming.HabboCamera
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger("Polar.Communication.Packets.Incoming.HabboCamera.PurchaseCameraPictureEvent");
        public async void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            await PolarEnvironment.SendMs2("", RoleplayManager.CDNSWF + "/newfoto/" + jsonInfo.preview, "¡Mira está foto que ha tomada por " + Session.GetHabbo().Username + "!", "En " + Session.GetHabbo().CurrentRoom.Name, Session.GetHabbo().Look, true);
            ...
            {
                log.Error($"An error occurred in PurchaseCameraPictureEvent: {ex}");
            }
        }
    }
}
