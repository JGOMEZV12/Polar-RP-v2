...
namespace Polar.Communication.Packets.Incoming.Catalog
{
    public class RedeemVoucherEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            if (!PolarEnvironment.GetGame().GetCatalog().GetVoucherManager().TryGetVoucher(VoucherCode, out Voucher))
            {
            ...
            {
                Session.SendNotification("Vaya, ¡este comprobante ha alcanzado el límite máximo de uso!");
                return;
            }
            ...
            if (GetRow != null)
            {
                Session.SendNotification("Ya usaste este código de código ¡lo siento!");
                return;
            }
            else
            {
