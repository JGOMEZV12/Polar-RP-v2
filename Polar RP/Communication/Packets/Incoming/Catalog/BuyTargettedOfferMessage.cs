...
namespace Polar.Communication.Packets.Incoming.Catalog
{
    internal class BuyTargettedOfferMessage : IPacketEvent
    {
...

        {
            ...
            var offer = PolarEnvironment.GetGame().GetTargetedOffersManager().TargetedOffer;
            ...
            if (extraMoneyCost > 0)
            {
                ...
                switch (offer.MoneyType)
                {
                    ...
                    case "eventos":
                        {
                            if (habbo.EventPoints < extraMoneyCost)
                            {
                                Session.SendMessage(new PurchaseErrorComposer(1));
                                return;
                            }
                            ...
                            //habbo.Diamonds -= extraMoneyCost;
                            Session.GetHabbo().EventPoints -= extraMoneyCost;
            ...
            TargetedOffers TargetedOffer = PolarEnvironment.GetGame().GetTargetedOffersManager().TargetedOffer;
                ...
                foreach (DataRow Row in count.Rows)
                {
...

                    {
                        using (var dbClient = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
                            dbClient.RunQuery("UPDATE users SET targeted_buy = targeted_buy +1 WHERE id = " + Session.GetHabbo().Id + ";");
                        foreach (var product in offer.Products)
                        {
                            ...
                            switch (product.ItemType)
                            {
                                ...
                                case "item":
                                    {
                                        ItemData item = null;
                                        if (!PolarEnvironment.GetGame().GetItemManager().GetItem(int.Parse(product.Item), out item)) return;
                                        if (item == null) return;
            ...
            TargetedOffers TargetedOffer2 = PolarEnvironment.GetGame().GetTargetedOffersManager().TargetedOffer;
            using (IQueryAdapter dbQuery2 = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                foreach (DataRow Row2 in count2.Rows)
                {
                    ...
                    {
                        //Session.SendMessage(PolarEnvironment.GetGame().GetTargetedOffersManager().TargetedOffer.Serialize());
                    }
                }
            }
            #endregion
