...
namespace Polar.Communication.Packets.Incoming.Rooms.Furni
{
    internal class OpenGiftEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            using (UserCache Purchaser = PolarEnvironment.GetGame().GetCacheManager().GenerateUser(PurchaserId))
            {
                if (Purchaser == null)
                {
                    Session.SendNotification("Oops! Appears there was a bug with this gift.\nWe'll just get rid of it for you.");
            ...
            if (!PolarEnvironment.GetGame().GetItemManager().GetItem(Convert.ToInt32(Data["base_id"]), out BaseItem))
            {
