        ...
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (Pet.PetData.AnyoneCanRide == 0 && Pet.PetData.OwnerId != User.UserId)
            {
                Session.SendNotification(
                    "No puede montar este caballo.\n El propietario de la mascota no ha seleccionado que nadie lo monte..");
                return;
            }
            ...
            if (Type)
            {
                if (Pet.RidingHorse)
                {
                    string[] Speech2 = PolarEnvironment.GetGame().GetChatManager().GetPetLocale().GetValue("pet.alreadymounted");
                    var RandomSpeech2 = new Random();
                    Pet.Chat(Speech2[RandomSpeech2.Next(0, Speech2.Length - 1)], false);
                }
                else if (User.RidingHorse)
                {
                    Session.SendNotification("¡Ya estás montando a caballo!");
                }
                else
                {
            ...
            else
            {
                ...
                {
                    Session.SendNotification("No pude desmontar este caballo. ¡No lo está montando!");
                }
            }
