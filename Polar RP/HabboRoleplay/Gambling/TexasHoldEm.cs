    ...
    public class TexasHoldEm
    {
        ...
        public void AddPlayerToGame(int UserId)
        {
            try
            {
                ...
                GameClient Player = PolarEnvironment.GetGame().GetClientManager().GetClientByUserID(UserId);
                ...
                {
                    Player.SendWhisper("¡No tienes suficiente dinero para jugar! El pote de arranque cuesta $" + String.Format("{0:N0}", this.JoinCost) + "!", 34);
                    return;
                }
                ...
                RoleplayManager.Shout(Player, "*Se une al juego Texas Hold'em como jugador " + Number + "*", 34);
                ...
                if (this.PlayerList.Values.Where(x => x != null && x.UserId > 0).ToList().Count == 3)
                {
                    ...
                    {
                        GameClient Client = PolarEnvironment.GetGame().GetClientManager().GetClientByUserID(this.PlayerList[1].UserId);
                        if (Client != null)
                            SendStartMessage("Por favor " + Client.GetHabbo().Username + " Haga rodar sus dados, dandole clic dos veces");
                    }
                }
            }
            catch (Exception e)
            ...
            try
            {
                ...
                lock (this.PlayerList)
                {
                    foreach (int UserId in this.PlayerList.Values.Where(x => x != null && x.UserId > 0).Select(x => x.UserId))
                    {
                        ...
                        Player.SendWhisper((Message == "" ? "¡El juego de Texas Hold'em está a punto de comenzar!" : Message), 38);
                    }
                }
            }
            catch (Exception e)
        ...
        public void SpawnStartingBet(int Number)
        {
            try
            {
                ...
                GameClient Client = PolarEnvironment.GetGame().GetClientManager().GetClientByUserID(Player.UserId);
        ...
        public void SpawnBetFurni(int Number, int Amount)
        {
            try
            {
                if (!RoleplayManager.GenerateRoom(this.RoomId, out Room Room) || Room.GetRoomItemHandler() == null || Room.GetGameMap() == null)
                    return;
                ...
                if (Fifties > 0)
                {
                    ...
                    {
                        // Emerald Bar
                        RoleplayManager.PlaceItemToRoom(null, 187378, 0, X, Y, Height, 0, false, this.RoomId, false, "0");
                        Height += 0.5;
                        Fifties--;
                    }
                }
                ...
                if (Tens > 0)
                {
                    ...
                    {
                        // Ruby Bar
                        RoleplayManager.PlaceItemToRoom(null, 187377, 0, X, Y, Height, 0, false, this.RoomId, false, "0");
                        Height += 0.5;
                        Tens--;
                    }
                }
                ...
                if (Fives > 0)
                {
                    ...
                    {
                        // Sapphire Bar
                        RoleplayManager.PlaceItemToRoom(null, 187381, 0, X, Y, Height, 0, false, this.RoomId, false, "0");
                        Height += 0.5;
                        Fives--;
                    }
                }
        ...
        public void RollDice(GameClient Session, Item Item, int Request)
        {
            try
            {
                ...
                {
                    Session.SendWhisper("¡No es tu turno de tirar tus dados!", 1);
                    return;
                }
        ...
        public void ChangeTurn()
        {
            try
            {
                ...
                if (PlayerId > 0)
                {
                    ...
                    {
                        if (GameSequence == 0)
                            SendStartMessage("Por favor " + Client.GetHabbo().Username + "'s  Gire para tirar sus dados!");
                        else if (GameSequence == 1)
                        {
                            SendStartMessage("por favor " + Client.GetHabbo().Username + "'s giro. ¿Quieres ':apostar' o ':pasar' para abandonar ':salirevento (fold)'?");
                            Client.SendWhisper("La apuesta mínima actual es de $" + String.Format("{0:N0}", this.MinimumBet(this.PlayersTurn)) + "! Solo tienes $" + String.Format("{0:N0}", this.PlayerList[this.PlayersTurn].TotalAmount) + " !", 1);
                        }
                        else if (GameSequence == 2)
                        {
                            SendStartMessage("Sr/a " + Client.GetHabbo().Username + "'s ¿Quiere ':apostar' o ':pasar'  ':dejarevento (fold)'?");
                            Client.SendWhisper("La apuesta mínima actual es $" + String.Format("{0:N0}", this.MinimumBet(this.PlayersTurn)) + "! Solo tienes $" + String.Format("{0:N0}", this.PlayerList[this.PlayersTurn].TotalAmount) + " !", 1);
                        }
                    }
                }
            }
        ...
        public void EndGame(int Winner)
        {
            try
            {
                ...
                {
                    GameClient Client = PolarEnvironment.GetGame().GetClientManager().GetClientByUserID(this.PlayerList[Winner].UserId);

                    if (Client != null && Client.GetRoleplay() != null && Client.GetHabbo() != null)
                    {
                        SendWinnerMessage("El ganador es: " + Client.GetHabbo().Username + " Quien gana el bote de $" + String.Format("{0:N0}", Prize) + "!");
        ...
        public void SendWinnerMessage(string Message)
        {
            try
            {
                if (RoleplayManager.GenerateRoom(this.RoomId, out Room Room) && Room.GetRoomUserManager() != null && Room.GetRoomUserManager().GetRoomUsers() != null)
                {
