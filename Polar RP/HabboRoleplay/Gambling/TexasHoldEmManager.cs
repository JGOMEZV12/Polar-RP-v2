...
namespace Polar.HabboRoleplay.Gambling
{
    public class TexasHoldEmManager
    {
        ...
        /// log4net
        /// </summary>
        private static readonly ILog log = LogManager.GetLogger("Polar.HabboRoleplay.Gambling.TexasHoldEmManager");
        ...
        public static void Initialize()
        {
            ...
            using (var dbClient = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (Table != null)
                {
                    foreach (DataRow Row in Table.Rows)
                    {
                        ...
                        #region Spawn Furni
                        if (RoleplayManager.GenerateRoom(RoomId, out Room Room) && Room.GetRoomItemHandler() != null)
                        {
           ...
           // log.Info("Loaded " + GameList.Count + " Texas Hold 'Em Games!");
            Out.WriteLine("Cargado(s): " + GameList.Count + " Texas Hold 'Em Games!", "Polar.HabboRoleplay", ConsoleColor.DarkGray);
        }
        ...
        public static void ClearOldData()
        {
            if (GameList.Count > 0)
            {
                foreach (TexasHoldEm Game in GameList.Values)
                {
                    ...
                    if (RoleplayManager.GenerateRoom(Game.RoomId, out Room Room))
                    {
                        #region Reset Game
                        Game.RemovePotFurni();
