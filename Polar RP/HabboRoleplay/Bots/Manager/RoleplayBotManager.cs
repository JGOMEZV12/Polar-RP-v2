...
using log4net;
using Polar.Core;
using Polar.HabboRoleplay.Bots.PetBots;
using Polar.HabboHotel.GameClients;
using System.Threading.Tasks;

namespace Polar.HabboRoleplay.Bots.Manager
{
    public class RoleplayBotManager
    {
        ...
        public static RoomUser DeployBotByID(int RoleplayBotsID, string SpawnType = "default", int RoomID = 0)
        {
            lock (SyncLock)
            {
                try
                {
                    ...
                    if (RoomID == 0)
                    {
                        if (!RoleplayManager.GenerateRoom(DeployingBot.SpawnId, out RoleplayBotsRoom, false))
                            return null;
                    }
                    else
                    {
                        if (!RoleplayManager.GenerateRoom(RoomID, out RoleplayBotsRoom, false))
                            return null;
        ...
        public static RoleplayBotAIType GetRoleplayBotAIType(string StringType)
        {
            switch (StringType.ToLower())
            {
                ...
                case "meca":
                case "mecanico":
                    return RoleplayBotAIType.MECANICO;
                default:
        ...
        public static RoleplayBotAI GetRoleplayBotAI(RoleplayBotAIType Type, int VirtualId)
        {
            switch (Type)
            {
                ...
                case RoleplayBotAIType.MECANICO:
                    return new RepairServerBot(VirtualId);
                default:
        ...
        public static bool TransportDeployedBot(RoomUser RoleplayBot, int NewRoomID, bool SaveDataToCache = true)
        {
            ...
            try
            {
                ...
                if (!RoleplayManager.GenerateRoom(NewRoomID, out Room NewRoom, false))
                    return false;
