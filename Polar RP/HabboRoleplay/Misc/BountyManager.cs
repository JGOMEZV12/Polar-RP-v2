...
namespace Polar.HabboRoleplay.Misc
{
    public class BountyManager
    {
        ...
        /// log4net
        /// </summary>
        private static readonly ILog log = LogManager.GetLogger("Polar.HabboRoleplay.Misc.BountyManager");
        ...
        public static void Initialize()
        {
            ...
            //log.Info("Cargada(s): " + BountyUsers.Count + " recompensas del usuario.");
            Out.WriteLine("Cargada(s): " + BountyUsers.Count + " recompensas del usuario.", "Polar.HabboRoleplay", ConsoleColor.DarkGray);
        }
        ...
        public static void RemoveBounty(int UserId, bool Expired = false)
        {
            ...
            if (!Expired)
            {
                try
                {
                    Habbo BountyOwner = PolarEnvironment.GetHabboById(Convert.ToInt32(Junk.AddedBy));
        ...
        public static void CheckBounty(GameClient Session, int UserId)
        {
            try
            {
                ...
                {
                    Session.SendWhisper("¡Vaya! Has reclamado " + PolarEnvironment.GetHabboById(UserId).Username + "'s Generosidad, pero su recompensa ha expirado!", 1);
                    return;
                }
            }
            catch { }
