...
namespace Polar.HabboRoleplay.Misc
{
    public class LotteryManager
    {
        ...
        /// log4net
        /// </summary>
        private static readonly ILog log = LogManager.GetLogger("Polar.HabboRoleplay.Misc.LotteryManager");
        ...
        public static void Initialize()
        {
            ...
            //log.Info("Cargado(s): " + LotteryTickets.Count + " tickets de loteria.");
            Out.WriteLine("Cargado(s): " + LotteryTickets.Count + " tickets de loteria.", "Polar.HabboRoleplay", ConsoleColor.DarkGray);
        }
        ...
        {
            GameClient Client = PolarEnvironment.GetGame().GetClientManager().GetClientByUserID(Winner);
            ...
            else
            {
                ...
                var User = PolarEnvironment.GetGame().GetCacheManager().GenerateUser(Winner);
        ...
        public static void SendWinnerAlert(string Winner)
        {
            lock (PolarEnvironment.GetGame().GetClientManager().GetClients)
            {
                foreach (var client in PolarEnvironment.GetGame().GetClientManager().GetClients.ToList())
                {
                    ...
                    client.SendWhisper("[ALERTA] [LOTERÍA] " + Winner + " ¡Acaba de ganar la lotería!", 33);
                }
            }
        }
