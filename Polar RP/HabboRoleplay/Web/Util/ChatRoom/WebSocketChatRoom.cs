...
namespace Polar.HabboRoleplay.Web.Util.ChatRoom
{
    public class WebSocketChatRoom : IDisposable
    {
        ...
        public WebSocketChatRoom(string ChatName, int ChatOwner, Dictionary<object, object> ChatValues, List<int> ChatAdmins, bool FromDB = false)
        {
            ...
            {
                this.Stop("¡Este chat fue actualizado! Por favor, vuelve a reunirte de nuevo.");
            }
        ...
        public void RefreshChatRoomData()
        {
            ...
            using (IQueryAdapter DB = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (TheChat != null)
                {
                    ...
                    bool Locked = PolarEnvironment.EnumToBool(Convert.ToString(TheChat["locked"]));
                ...
                foreach (DataRow ChatRoomData in ChatRoomsData.Rows)
                {
                    ...
                    if (DataExpire <= PolarEnvironment.GetUnixTimestamp())
                    {
        ...
        public bool ExploitingAction(GameClient User)
        {
            if (!this.ChatUsers.ContainsKey(User))
            {
                ...
                {
                    User.SendWhisper("No se puede hacer esto ya que no estás en este chat", 1);
                }
                else
                {
                    User.GetRoleplay().SendTopAlert("No se puede hacer esto ya que no estás en este chat");
                }
        ...
        {
            this.ChatLogs.TryAdd(this.ChatLogs.Count + 1, new Dictionary<string, string>() { { "chat_name", this.ChatName }, { "user_id", Convert.ToString(User.GetHabbo().Id) }, { "chat_message", Message }, { "timestamp", Convert.ToString(PolarEnvironment.GetUnixTimestamp()) } });
        }
        ...
        public void SaveChatRoomData()
        {
            try
            {
                ...
                using (IQueryAdapter DB = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    DB.AddParameter("locked", PolarEnvironment.BoolToEnum(Convert.ToBoolean(this.ChatValues["locked"])));
                    DB.AddParameter("chatname", this.ChatName);
                    DB.RunQuery();
                }
                #endregion
        ...
        public bool OnUserJoin(GameClient User)
        {
            ...
            if (User.GetRoleplay().WebSocketConnection == null)
            {
                User.SendWhisper("Tu conexión de websocket está desconectada. Póngase en contacto con un miembro del personal si el problema persiste", 1);
                return false;
            }
        ...
        public void WarnUser(GameClient User, string Message)
        {
            ...
            PolarEnvironment.GetGame().GetWebEventManager().SendDataDirect(User, JsonConvert.SerializeObject(new Dictionary<object, object>()
            {
                { "event", "chatManager" },
                { "action", "newwarnchat" },
        ...
        public void BeginChatJoin(GameClient User)
        {
            ...
            PolarEnvironment.GetGame().GetWebEventManager().ExecuteWebEvent(User, "event_chatroom", JsonConvert.SerializeObject(new Dictionary<object, object>()
            {
                { "chatname", ChatName },
                { "action", "joinedchat" }
            }));
        ...
        public string ReturnOwnersFigure()
        {
            ...
            using (UserCache Member = PolarEnvironment.GetGame().GetCacheManager().GenerateUser(this.ChatOwner))
            {
        ...
        public void BroadCastNewChat(GameClient User, Dictionary<object, object> Params)
        {
            ...
            foreach (GameClient ChatUser in this.ChatUsers.Keys)
            {
                ...
                ChatUser.SendMessage(new RoomNotificationComposer("text_message", "message", "Recibió un nuevo mensaje del '" + this.ChatName + "' grupo de whatsapp"));
            }
            #endregion
        }
        ...
        public void BroadCastChatHTML(GameClient User, Dictionary<object, object> Params)
        {
            ...
            if (User.GetRoleplay().WebSocketConnection == null)
            {
                User.SendWhisper("Your websocket connection is currently offline. Contact a staff member if this problem persists..", 1);
                return;
            }
        ...
        public void BroadCastChatData(GameClient User, string Data, bool SendToMe = false)
        {
            ...
            foreach (GameClient ChatUser in this.ChatUsers.Keys)
            {
                ...
                PolarEnvironment.GetGame().GetWebEventManager().SendDataDirect(ChatUser, Data);       
            }
        }
        ...
        public void SaveNewChat()
        {
            ...
            using (IQueryAdapter DB = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (CheckRow == null)
                {
                    ...
                    DB.SetQuery("INSERT INTO rp_chat_rooms(owner_id, name, password, locked, gang_id, admins) VALUES('" + this.ChatOwner + "', @chatname, '" + this.ChatValues["password"] + "', '" + PolarEnvironment.BoolToEnum(Convert.ToBoolean(this.ChatValues["locked"])) + "', '" + this.ChatValues["gang"] + "', '" + String.Join(":", this.ChatAdmins) + "')");
                    DB.AddParameter("chatname", this.ChatName);
                    DB.RunQuery();
            ...
            lock (this.ChatUsers)
            {
                ...
                foreach (GameClient User in this.ChatUsers.Keys)
                {
                    ...
                    if (User.GetRoleplay().WebSocketConnection != null)
                    {
                        if (!User.LoggingOut)
                        {
                            PolarEnvironment.GetGame().GetWebEventManager().SendDataDirect(User, Newtonsoft.Json.JsonConvert.SerializeObject(new Dictionary<object, object>() { { "event", "chatManager" }, { "action", "disconnectchat" }, { "chatname", this.ChatName } }));
