...
namespace Polar.HabboRoleplay.Food
{
    class FoodManager
    {
        /// <summary>
            ...
            using (IQueryAdapter DB = PolarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (Foods != null)
                {
                    foreach (DataRow Food in Foods.Rows)
                    {
                        ...
                        int Alcohol = Convert.ToInt32(Food["alcohol"]);
                        int Hunger = Convert.ToInt32(Food["hunger"]);
                        string ServeText = Food["serve_text"].ToString();
                        string EatText = Food["eat_text"].ToString();
                        bool Servable = PolarEnvironment.EnumToBool(Food["servable"].ToString());

                        Food newFood = new Food(Name, Type, ItemId, ExtraData, Cost, Health, Energy, Alcohol, Hunger, ServeText, EatText, Servable);
                        FoodList.TryAdd(Name, newFood);
                    }
                }
            }

           // log.Info("Cargada " + FoodList.Count + " comidas.");
            Out.WriteLine("Carga: " + FoodList.Count + " comida(s).", "Polar.HabboRoleplay.Food.FoodManager", ConsoleColor.DarkGray);
        }
        ...
        public static Food GetFoodTwo(string ItemName)
        {
            try
            {
                Food thefood = null;
        ...
        public static string GetServableItems(HabboHotel.GameClients.GameClient Session)
        {
            ...
            {
                if (!HabboHotel.Groups.GroupManager.HasJobCommand(Session, item.Type))
                    continue;
