...
namespace Polar.HabboRoleplay.Farming
{
    class FarmingManager
    {
        ...
        /// log4net
        /// </summary>
        private static readonly ILog log = LogManager.GetLogger("Polar.HabboRoleplay.Food.FarmingManager");
        ...
        public static void Initialize()
        {
            ...
            //log.Info("Loaded " + FarmingItems.Count + " farming items.");
            Out.WriteLine("Cargado(s): " + FarmingItems.Count + " elementos de cultivo.", "Polar.HabboRoleplay.Food.FarmingManager", ConsoleColor.DarkGray);
            #endregion
            ...
            //log.Info("Loaded " + FarmingSpaces.Count + " farming rentable spaces.");
            Out.WriteLine("Cargado(s): " + FarmingSpaces.Count + " espacios rentables de agricultura.", "Polar.HabboRoleplay.Food.FarmingManager", ConsoleColor.DarkGray);
            #endregion
        }
            ...
            try
            {
                ...
                if (Session != null && Session.GetRoleplay() != null)
                {
                    ...
                    if (LevelUp(Session))
                    {
                        ...
                        Session.SendWhisper("Usted acaba de nivelado hasta su cultivo! Su cultivo es ahora Nivel: " + Session.GetRoleplay().FarmingStats.Level + ".");
                    }
                    else
                        Session.SendWhisper("Has recibido " + Amount + " EXP hacia su próximo nivel de cultivo! Necesitas" + (levels[Session.GetRoleplay().FarmingStats.Level + 1] - Session.GetRoleplay().FarmingStats.Exp) + " Llegar al nivel de cultivo " + (Session.GetRoleplay().FarmingStats.Level + 1), 1);
                }
            }
            catch
            {
        ...
        public static void EquipWateringCan(GameClient Session, Item Item)
        {
            ...
            {
                Session.SendWhisper("Lo siento, sólo un agricultor puede usar la regadera", 1);
                return;
            }

            if (Session.GetRoleplay().WateringCan)
            {
                Session.SendWhisper("Ya está equipando una regadera", 1);
                return;
            }
            ...
            Session.Shout("*Agarra una regadera del suelo y se prepara para comenzar a cultivar*", 4);
        }
        ...
        public static void PlantSeed(GameClient Session, Item Item)
        {
            ...
            if (!GroupManager.HasJobCommand(Session, "farming"))
            {
                Session.SendWhisper("Lo siento, sólo un agricultor puede plantar semillas!", 1);
            ...
            {
                Session.SendWhisper("Usted debe estar trabajando para plantar semillas", 1);
                Session.GetRoleplay().CooldownManager.CreateCooldown("farming", 500);
                return;
            }
            ...
            {
                Session.SendWhisper("¡No has seleccionado una semilla para plantar todavía! Utilice el comando ':plantar <id>' para seleccionar uno", 1);
                Session.GetRoleplay().CooldownManager.CreateCooldown("farming", 500);
                return;
            }

            ItemData Furni;
            if (!PolarEnvironment.GetGame().GetItemManager().GetItem(Session.GetRoleplay().FarmingItem.BaseItem, out Furni))
            {
                Session.SendWhisper("Lo sentimos, pero la planta que ha seleccionado no es válida. Utilice el comando ':plantar <id>' para seleccionar otro!", 1);
                Session.GetRoleplay().CooldownManager.CreateCooldown("farming", 500);
                return;
            }
            ...
            if (Amount <= 0)
            {
                Session.SendWhisper("Parece que te has quedado sin " + Furni.PublicName + " Semillas!", 1);
                Session.GetRoleplay().CooldownManager.CreateCooldown("farming", 500);
                return;
            }
